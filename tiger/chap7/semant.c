...
static struct expty transExp(Tr_level level, Tr_exp breakk, S_table v, S_table t, A_exp e){
	...
	case A_forExp: {
			struct expty final = transVar(level, breakk, v, t, e->u.forr.var);
			struct expty final6 = transExp(level, breakk, v, t, e->u.forr.lo);
			struct expty final7 = transExp(level, breakk, v, t, e->u.forr.hi);
			if (final.ty->kind!=Ty_int || final6.ty->kind != Ty_int || final7.ty->kind != Ty_int) {
				EM_error(e->pos, "int required");
			}
			Tr_exp done = Tr_doneExp();
			struct expty body = transExp(level, done, v, t, e->u.forr.body);
			return expTy(Tr_forExp(final.exp, final6.exp, final7.exp, body.exp, done), Ty_Void());
	}
	case A_letExp: {
		A_decList decs;
		Tr_expList l = NULL;
		...
		if (isOutmost(level))
		{
			main_label = getLevelName(level);
			Tr_procEntryExit(level, final.exp, Tr_formals(level));
		}	
	...
	case A_opExp: {
		...
		if (0 <= oper && oper < 6) {/* check +,-,*,/ */
			...
			return expTy(Tr_noExp(), Ty_Int());
		} else if (5 < oper && oper < 12) {
			Tr_exp translation = Tr_noExp();
			if (oper == 6 || oper == 7) {/*check record type can be nil(=, <>)*/
				switch(left.ty->kind) {
				case Ty_int:
...
static Tr_exp transDec(Tr_level level, Tr_exp breakk, S_table v, S_table t, A_dec d) {
	...
	case A_varDec:
		...
		Tr_exp tmp = Tr_assignExp(Tr_simpleVar(ac, level), final.exp);
		if(isOutmost(level))
			Tr_decEntryExit(level, tmp, Tr_formals(level));
		return tmp;
	case A_functionDec:
