...
F_frag F_ProcFrag(T_stm body, F_frame frame, Temp_label name) {
	F_frag pfrag = checked_malloc(sizeof(*pfrag));
	pfrag->kind = F_procFrag;
	pfrag->u.proc.body = body;
	pfrag->u.proc.frame = frame;
	pfrag->u.proc.name = name;
	return pfrag;
}

F_frag F_DecFrag(T_stm body, F_frame frame) {
	F_frag pfrag = checked_malloc(sizeof(*pfrag));
	pfrag->kind = F_decFrag;
	pfrag->u.proc.body = body;
	pfrag->u.proc.frame = frame;
	return pfrag;
}
...
T_exp F_ExpUp( T_exp framePtr) { /* visit frame-offs addr & get content */
